...
{
    class UpdateDailyExecutionActuals
    {
        ...
        public UpdateDailyExecutionActuals()
        {

        }

        public UpdateDailyExecutionActuals(Properties props)
        {
        ...
        public void UpdateTestResultsSingle(List<TestCase> testCases)
        {
            ...
            Console.Write("Updating Test Execution Input Data:  ");
            ...
            using (var progress = new ProgressBar())
            {
                ...
                foreach (TestCase tempTestCase in testCases)
                {
                    ...
                    {
                        _scriptWorksheet.Cells[idToRowMapping[tempTestCase.TestCaseId], 18].Value = tempTestCase.CurrentIteration;
                        _scriptWorksheet.Cells[idToRowMapping[tempTestCase.TestCaseId], 31].Value = tempTestCase.TestCasePath;
        ...
        public void UpdateTestResultsActuals(List<TestCase> testCases)
        {
            ...
            //_executionWorksheet.DeleteRow(2, rowCount);

            int rowCount = 2;

            //SortedDictionary<DateTime, TestCase> dateTimeSorting = new SortedDictionary<DateTime, TestCase>();
            //foreach(TestCase curr in testCases)
            //{
            //    dateTimeSorting[curr.CurrentTestCaseResult.ResultDT] = curr;
            //}
            ...
            using (var progress = new ProgressBar())
            {
                ...
                foreach (TestCase tempTestCase in testCases)
                {
                    ...
                    //        _executionWorksheet.Cells[currRowCount, 28].Value = UpdatedTestCase.TestSuiteId + "" + UpdatedTestCase.TestCaseName;
                    ...
                    //{
                        ...
                        if (!_mappedScriptIdToTestRunId.ContainsKey(currHashValue) && tempTestCase.CurrentTestCaseResult.Result != "In Progress")
                        {
                            ...
                            _executionWorksheet.Cells[currRowCount, 17].Value = tempTestCase.OriginalIteration;
                            _executionWorksheet.Cells[currRowCount, 18].Value = tempTestCase.CurrentIteration;

                            _executionWorksheet.Cells[currRowCount, 21].Value = tempTestCase.CurrentTestCaseResult.RunByName;
                            _executionWorksheet.Cells[currRowCount, 22].Value = tempTestCase.CurrentTestCaseResult.Result;

                            _executionWorksheet.Cells[currRowCount, 28].Value = tempTestCase.TestCaseId + "" +
                                                                                    tempTestCase.TestSuiteId + "" + 
                                                                                    tempTestCase.TestCaseName;
                            _executionWorksheet.Cells[currRowCount, 29].Value = tempTestCase.CurrentTestCaseResult.ResultDT.ToString("MM/dd/yyyy");
                            _executionWorksheet.Cells[currRowCount, 30].Value = tempTestCase.CurrentTestCaseResult.TestRunId;
                            _executionWorksheet.Cells[currRowCount, 31].Value = tempTestCase.TestCasePath;
        ...
        public void SetupMapping()
        {
            ...
            while (_executionWorksheet.Cells[rowCount, 1] != null && _executionWorksheet.Cells[rowCount, 1].Text != "")
            {
                var clearString = "A" + rowCount + ":AE" + rowCount;
                _executionWorksheet.Cells[clearString].Clear();
                //_executionWorksheet.DeleteRow(rowCount);
                rowCount += 1;
            }

            //_executionWorksheet.DeleteRow(2, rowCount - 1);
        ...
        public TestCase GatherTestScriptInformation(TestCase testCase)
        {
            ...
            if (_mappedScriptIdToRow.ContainsKey(testCase.TestCaseId))
            {
                ...
                if (_scriptWorksheet.Cells[testScriptRow, 17] != null && _scriptWorksheet.Cells[testScriptRow, 17].Text != "")
                {
                    testCase.OriginalIteration = Convert.ToInt32(_scriptWorksheet.Cells[testScriptRow, 17].Text);
                }
                if (_scriptWorksheet.Cells[testScriptRow, 18] != null && _scriptWorksheet.Cells[testScriptRow, 18].Text != "")
                {
